...
// ---------- 队伍成员管理 ----------
// key: teamId, value: Set of player names (实际用数组存储)
let teamMembers = {};
PRESET_TEAMS.forEach(t => { teamMembers[t.id] = []; });

// ---------- 问答历史记录 ----------
let answerHistory = []; // 每条记录包含: { id, timestamp, activityType, teamId, teamName, playerName, question, selectedAnswer, isCorrect, scoreDelta }
...
app.post('/api/team-members/:teamId', (req, res) => {
    ...
    if (!teamMembers[teamId].includes(playerName)) {
        teamMembers[teamId].push(playerName);
    }
...
app.delete('/api/team-members/:teamId', (req, res) => {
    ...
    if (teamMembers[teamId]) {
        teamMembers[teamId] = teamMembers[teamId].filter(n => n !== playerName);
    }
    res.json({ success: true });
});
            ...
            if (data.type === 'HOST_ACTION') {
                ...
                if (action === 'selectActivity') {
                    if (payload === 'rush') {
                        ...
                        gameState.rush = { roundState: 'IDLE', currentQuestion: null, rushEndTime: 0, answerEndTime: 0, buzzerPlayerId: null, buzzerPlayerName: null, correctAnswer: null };
                ...
                if (gameState.currentActivity === 'rush') {
                    const result = handleRushAnswer(data.playerId, data.playerName, data.answerIndex);
                    if (result) ws.send(JSON.stringify({ type: 'ANSWER_RESULT', result }));
                } else {
                    const result = handleMutualAnswer(data.playerId, data.playerName, data.answerIndex);
                    if (result) ws.send(JSON.stringify({ type: 'ANSWER_RESULT', result }));
                }
                ...
                if (gameState.currentActivity === 'rush' && gameState.rush.roundState === 'ANSWERING') {
                    ...
                    if (p) {
                        ...
                        gameState.lastAnswerResult = { teamId: p.id, teamName: p.name, playerName: gameState.rush.buzzerPlayerName, isCorrect: false, message: `⏰ ${p.name}（${gameState.rush.buzzerPlayerName}）答题超时，不得分`, timestamp: Date.now() };
                        broadcastState();
                        setTimeout(() => {
                            if (gameState.currentActivity === 'rush' && gameState.rush.roundState === 'FINISHED') {
                                gameState.rush.roundState = 'IDLE';
